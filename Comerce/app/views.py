...
def Register(request):
    ...
    if username in usernames:
        ...
        payload = "sender_id=FSTSMS&message=" + "OTP FOR Registration in E commerce:" + OTP + "&language=english&route=p&numbers=" + str(
            contact)
        headers = {
        ...
        RegisterDetails(name=name, contact=contact, email=email, username=username, password=password,
                        confirm_password=confirm_password,otp=OTP).save()
        # c = categeroy.objects.all()
        # p = product.objects.all()
        return render(request, "INDEX.html", {"type": "otpvalidation","name":name})
def Forget(request):
    type=request.GET.get("type")
...
def AddTocart(request):
    ...
    p=product.objects.all()
    c=categeroy.objects.all()
    status = request.session["status"]=True
...
def OtpValidate(request):
    ...
    return render(request,"INDEX.html",{"type":"productdetails","name":name})
